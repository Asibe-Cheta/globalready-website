...
export default function EditInDemandRolePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError('')
      try {
        const res = await fetch(`/api/admin/in-demand-roles/${id}`, { credentials: 'include' })
        if (!res.ok) {
          const data = await res.json().catch(() => ({}))
          throw new Error(data.error || 'Failed to load role')
        }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/in-demand-roles/${id}`, {
        method: 'PATCH',
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-3xl font-bold text-white">Edit in-demand role</h1>
        ...
        <div className="bg-[#1a2432] border border-slate-700/50 rounded-2xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input list="icon-list-edit" value={form.icon} onChange={(e) => update('icon', e.target.value)} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" />
              <datalist id="icon-list-edit">
          ...
          <div>
            ...
            <textarea value={form.reason} onChange={(e) => update('reason', e.target.value)} rows={3} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" />
          </div>
