    ...
    async function load() {
      ...
      try {
        ...
        setForm({
          ...
          category: c.category ?? '',
          path_category: c.path_category ?? '',
          icon: c.icon ?? 'code',
          tint_color: c.tint_color ?? '#0d6cf2',
          display_order: c.display_order != null ? String(c.display_order) : '0',
          duration: c.duration ?? '',
        ...
        <div className="bg-[#1a2432] border border-slate-700/50 rounded-2xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input value={form.icon} onChange={(e) => update('icon', e.target.value)} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" />
            </div>
            <div>
