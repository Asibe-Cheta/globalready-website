...
export default function NewCoursePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <h1 className="text-3xl font-bold text-white">Add course</h1>
      <p className="text-slate-400 text-sm">Courses are stored in Supabase and available to the mobile app via the same Admin API.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-[#1a2432] border border-slate-700/50 rounded-2xl p-6 space-y-4">
          ...
          <div>
            ...
            <input value={form.subtitle} onChange={(e) => update('subtitle', e.target.value)} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" placeholder="e.g. Achieve Band 7+ in 8 weeks" />
          </div>
          <div>
        ...
        <div className="bg-[#1a2432] border border-slate-700/50 rounded-2xl p-6 space-y-4">
          ...
          <div>
            ...
            <input value={form.image_url} onChange={(e) => update('image_url', e.target.value)} type="url" className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" placeholder="https://..." />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Thumbnail URL</label>
            <input value={form.thumbnail_url} onChange={(e) => update('thumbnail_url', e.target.value)} type="url" className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" placeholder="https://..." />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
        ...
        <div className="bg-[#1a2432] border border-slate-700/50 rounded-2xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">Price (e.g. 99.00 for $99)</label>
              <input type="number" min={0} step={0.01} value={form.price} onChange={(e) => update('price', e.target.value)} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" placeholder="0 = free" />
            </div>
            <div>
        ...
        <div className="bg-[#1a2432] border border-slate-700/50 rounded-2xl p-6 space-y-4">
          ...
          <div>
            ...
            <textarea value={form.syllabus} onChange={(e) => update('syllabus', e.target.value)} rows={6} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white font-mono text-sm" placeholder='[{"week":1,"title":"Week 1","topics":["Topic A"]}]' />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Prerequisites (one per line)</label>
            <textarea value={form.prerequisites} onChange={(e) => update('prerequisites', e.target.value)} rows={2} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" placeholder="Basic English" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Learning outcomes (one per line)</label>
            <textarea value={form.learning_outcomes} onChange={(e) => update('learning_outcomes', e.target.value)} rows={3} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" placeholder="Score Band 7+" />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Tags (comma-separated)</label>
            <input value={form.tags} onChange={(e) => update('tags', e.target.value)} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" placeholder="ielts, english, test-prep" />
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <button type="submit" disabled={loading} className="px-6 py-2 bg-[#0d6cf2] text-white rounded-lg font-medium hover:bg-blue-600 disabled:opacity-50 flex items-center gap-2">
            {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : null}
            Create course
          </button>
          <Link href="/admin/courses" className="px-6 py-2 bg-[#223249] text-slate-300 rounded-lg font-medium hover:bg-[#223249]/80">
            Cancel
          </Link>
        </div>
      </form>
