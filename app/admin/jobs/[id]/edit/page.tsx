...
export default function EditJobPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (cancelled) return
        const pd = j.posted_date ? new Date(j.posted_date).toISOString().slice(0, 10) : ''
        setForm({
          title: j.title ?? '',
          company: j.company ?? '',
          country: j.country ?? '',
          ...
          posted_date: pd,
          requirements: j.requirements && typeof j.requirements === 'object' ? JSON.stringify(j.requirements, null, 2) : '{}',
  ...
  return (
    <div className="space-y-6">
      <Link href={`/admin/jobs/${id}`} className="inline-flex items-center gap-2 text-slate-400 hover:text-white">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-[#1a2432] border border-slate-700/50 rounded-2xl p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input value={form.sector} onChange={(e) => update('sector', e.target.value)} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" />
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
            ...
            <div>
              ...
              <input value={form.salary_range} onChange={(e) => update('salary_range', e.target.value)} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <textarea value={form.requirements} onChange={(e) => update('requirements', e.target.value)} rows={6} className="w-full px-4 py-2 bg-[#223249] border border-slate-700/50 rounded-lg text-white font-mono text-sm" />
          </div>
        </div>
        ...
        <div className="flex gap-4">
          ...
          <Link href={`/admin/jobs/${id}`} className="px-6 py-2 bg-[#223249] text-slate-300 rounded-lg font-medium hover:bg-[#223249]/80">
            Cancel
          </Link>
        </div>
      </form>
