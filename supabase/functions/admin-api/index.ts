// Supabase Edge Function — deploy with: supabase functions deploy admin-api --project-ref bwgqzoplcgxguylerqsn
// Source of truth: keep in sync with root admin_api/index.ts (or this file is the canonical one)
// @ts-nocheck
...
// ============================================================
// Route handlers (same as admin_api/index.ts)
// ============================================================
...
async function getUsers(url: URL) {
  ...
  const [cvsResult, paymentsResult, registrationsResult] = await Promise.all([
    ...
    supabase.from('payments').select('user_id, amount, status').in('user_id', userIds).eq('status', 'successful'),
    supabase.from('course_registrations').select('user_id').in('user_id', userIds),
  ]);
...
async function getPayments(url: URL) {
  ...
  if (error) return errorResponse(error.message, 500);
  return jsonResponse({ payments: data, pagination: { page, limit, total: count || 0 } });
}
...
async function getRegistrations(url: URL) {
  ...
  const offset = (page - 1) * limit;
  const courseId = url.searchParams.get('course_id');
  let query = supabase.from('course_registrations').select('*, courses(title, category)', { count: 'exact' });
  if (courseId) query = query.eq('course_id', courseId);
  const { data, error, count } = await query.order('created_at', { ascending: false }).range(offset, offset + limit - 1);
  if (error) return errorResponse(error.message, 500);
  return jsonResponse({ registrations: data, pagination: { page, limit, total: count || 0 } });
}
...
async function updateRegistrationStatus(id: string, body: any) {
  ...
  const { data, error } = await supabase.from('course_registrations').update(updates).eq('id', id).select().single();
  if (error) return errorResponse(error.message, 500);
  return jsonResponse(data);
}
...
async function getJobs(url: URL) {
  ...
  const { data, error, count } = await query
    ...
    .range(offset, offset + limit - 1);
  if (error) return errorResponse(error.message, 500);
  return jsonResponse({ jobs: data, pagination: { page, limit, total: count || 0 } });
}
...
async function createJob(body: any) {
  ...
  const { data, error } = await supabase.from('jobs').insert({
    title, company, country: country || null, city: city || null, job_type: job_type || null, sector: sector || null,
...
async function createInDemandRole(body: any) {
  ...
  if (!title) return errorResponse('Title is required');
  const { data, error } = await supabase.from('in_demand_roles').insert({
    rank: parseInt(String(rank), 10), title, icon: icon || 'code', accent_color: accent_color || '#3b82f6', reason: reason || null, is_active: is_active !== false,
  }).select().single();
  if (error) return errorResponse(error.message, 500);
  return jsonResponse(data, 201);
}
...
async function getSettings() {
  const supabase = getSupabase();
  const { data, error } = await supabase.from('admin_settings').select('key, value').in('key', ['admin_emails']);
  if (error) return errorResponse(error.message, 500);
...
async function updateSettings(body: { admin_emails?: string[] }) {
  ...
  const supabase = getSupabase();
  const { error } = await supabase.from('admin_settings').upsert({ key: 'admin_emails', value: emails, updated_at: new Date().toISOString() }, { onConflict: 'key' });
  if (error) return errorResponse(error.message, 500);
...
async function getAssessments(url: URL) {
  ...
  const offset = (page - 1) * limit;
  const status = url.searchParams.get('status');
  let query = supabase.from('assessments').select('id, user_id, target_country, job_sector, current_status, years_experience, education_level, match_score, eligibility_status, status, created_at, profiles:user_id(full_name, email)', { count: 'exact' });
  if (status) query = query.eq('status', status);
  const { data, error, count } = await query.order('created_at', { ascending: false }).range(offset, offset + limit - 1);
  if (error) return errorResponse(error.message, 500);
  return jsonResponse({ assessments: data, pagination: { page, limit, total: count || 0 } });
}
...
Deno.serve(async (req) => {
  ...
  let path = url.pathname.replace(/^.*\/admin-api\/?/i, '').replace(/\/$/, '').replace(/^\//, '');
  ...
  try {
    ...
    if (path === 'courses' && method === 'GET') return getCourses(url);
    if (path.match(/^courses\/[a-f0-9-]+$/) && method === 'GET') return getCourseById(path.split('/')[1]);
    if (path === 'courses' && method === 'POST') return createCourse(body);
    if (path.match(/^courses\/[a-f0-9-]+$/) && (method === 'PUT' || method === 'PATCH')) return updateCourse(path.split('/')[1], body);
    if (path.match(/^courses\/[a-f0-9-]+$/) && method === 'DELETE') return deleteCourse(path.split('/')[1]);

    if (path === 'virtual-sessions' && method === 'GET') return getVirtualSessions(url);
    if (path.match(/^virtual-sessions\/[a-f0-9-]+$/) && method === 'GET') return getVirtualSessionById(path.split('/')[1]);
    if (path === 'virtual-sessions' && method === 'POST') return createVirtualSession(body);
    if (path.match(/^virtual-sessions\/[a-f0-9-]+$/) && (method === 'PUT' || method === 'PATCH')) return updateVirtualSession(path.split('/')[1], body);
    if (path.match(/^virtual-sessions\/[a-f0-9-]+$/) && method === 'DELETE') return deleteVirtualSession(path.split('/')[1]);

    if (path === 'dashboard/stats' && method === 'GET') return getDashboardStats();
    if (path === 'dashboard/revenue' && method === 'GET') return getRevenueOverTime(url);
    if (path === 'dashboard/skills' && method === 'GET') return getSkillsInsights();
    if (path === 'dashboard/user-growth' && method === 'GET') return getUserGrowth(url);

    if (path === 'users' && method === 'GET') return getUsers(url);
    if (path.match(/^users\/[a-f0-9-]+$/) && method === 'GET') return getUserById(path.split('/')[1]);

    if (path === 'payments' && method === 'GET') return getPayments(url);

    if (path === 'registrations' && method === 'GET') return getRegistrations(url);
    if (path.match(/^registrations\/[a-f0-9-]+$/) && (method === 'PUT' || method === 'PATCH')) return updateRegistrationStatus(path.split('/')[1], body);

    if (path === 'assessments' && method === 'GET') return getAssessments(url);

    if (path === 'settings' && method === 'GET') return getSettings();
    if (path === 'settings' && (method === 'PATCH' || method === 'PUT')) return updateSettings(body);

    if (path === 'jobs' && method === 'GET') return getJobs(url);
    if (path.match(/^jobs\/[a-f0-9-]+$/) && method === 'GET') return getJobById(path.split('/')[1]);
    if (path === 'jobs' && method === 'POST') return createJob(body);
    if (path.match(/^jobs\/[a-f0-9-]+$/) && (method === 'PUT' || method === 'PATCH')) return updateJob(path.split('/')[1], body);
    if (path.match(/^jobs\/[a-f0-9-]+$/) && method === 'DELETE') return deleteJob(path.split('/')[1]);

    if (path === 'in-demand-roles' && method === 'GET') return getInDemandRoles(url);
    if (path.match(/^in-demand-roles\/[a-f0-9-]+$/) && method === 'GET') return getInDemandRoleById(path.split('/')[1]);
    if (path === 'in-demand-roles' && method === 'POST') return createInDemandRole(body);
    if (path.match(/^in-demand-roles\/[a-f0-9-]+$/) && (method === 'PUT' || method === 'PATCH')) return updateInDemandRole(path.split('/')[1], body);
    if (path.match(/^in-demand-roles\/[a-f0-9-]+$/) && method === 'DELETE') return deleteInDemandRole(path.split('/')[1]);
